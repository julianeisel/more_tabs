...
static void widget_tab(uiBut *but, uiWidgetColors *wcol, rcti *rect, int UNUSED(state), int roundboxalign)
{
	...
	/* space between tabs */
	if (but->drawflag & UI_BUT_SEP)
		BLI_rcti_resize(rect, BLI_rcti_size_x(rect) - 0.06f * U.dpi, BLI_rcti_size_y(rect));
	else
		/* a little bit space is always wanted */
		BLI_rcti_resize(rect, BLI_rcti_size_x(rect) - 0.02f * U.dpi, BLI_rcti_size_y(rect));

	BLI_rcti_translate(rect, -(0.25f * U.widget_unit), 0.2f * U.widget_unit);

	/* copy the new rect to the button's rect */
	BLI_rctf_resize(&but->rect, BLI_rcti_size_x(rect), BLI_rcti_size_y(rect));
	BLI_rctf_translate(&but->rect, -(0.25f * U.widget_unit), 0.2f * U.widget_unit);
